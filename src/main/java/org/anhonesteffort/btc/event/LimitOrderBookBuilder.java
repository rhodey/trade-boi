  ...
  protected void onEvent(OrderEvent event) throws OrderEventException {
    switch (event.getType()) {
      ...
      case LIMIT_DONE:
        ...
        if (limitDone.isPresent() && event.getSize() <= 0 && isGreaterOrEqual(limitDone.get().getSizeRemaining(), 0)) {
        ...
        } else if (limitDone.isPresent() && !isEqual(limitDone.get().getSizeRemaining(), event.getSize())) {
